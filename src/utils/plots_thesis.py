...
PLOT_LABEL_BY_METRIC = {
    'sanity_test_loss': 'Centralizado',
    'federated_test_loss': 'Federado',
    'sanity_test_perplexity': 'Centralizado',
    'federated_test_perplexity': 'Federado',
    'sanity_exact_match_rate': 'Centralizado',
    'federated_exact_match_rate': 'Federado',
    'sanity_partial_match_rate': 'Centralizado',
    'federated_partial_match_rate': 'Federado',
    'sanity_reference_secret_exact_match_rate': 'Centralizado',
    'federated_reference_secret_exact_match_rate': 'Federado',
    'sanity_reference_secret_partial_match_rate': 'Centralizado',
    'federated_reference_secret_partial_match_rate': 'Federado',
    'sanity_target_exact_match_rate': 'Centralizado',
    'federated_target_exact_match_rate': 'Federado',
    'sanity_structured_entity_generation_rate': 'Centralizado',
    'federated_structured_entity_generation_rate': 'Federado',
    'sanity_canary_recovery_rate': 'Centralizado',
    'federated_canary_recovery_rate': 'Federado',
}

THESIS_FIGURE_WIDTH = 6.3
THESIS_FIGURE_HEIGHT = 3.9
THESIS_DPI = 400
THESIS_COLORS = ('#1b4f72', '#b03a2e', '#117a65', '#7d6608')
THESIS_HATCHES = ('', '///', '...', 'xxx')
THESIS_FONT_FAMILY = 'serif'


def configure_thesis_plot_style() -> None:
    """Configura estilo consistente para figuras de tese.

    O objetivo é produzir figuras com legibilidade alta em impressão,
    boa reprodução em tons de cinza e tipografia compatível com texto acadêmico.
    """
    plt.rcParams.update({
        ...
        'font.family': THESIS_FONT_FAMILY,
        'font.size': 10,
        'axes.titlesize': 11,
        'axes.labelsize': 10,
...
def _infer_tick_decimals(metric_names: Sequence[str]) -> int:
    """Escolhe número de casas decimais adequado ao tipo de métrica."""
    joined = ' '.join(metric_names)
    if 'perplexity' in joined or 'loss' in joined:
        return 1
    if 'rate' in joined:
        return 2
    return 2
...
def _compute_y_axis_upper_bound(values: Sequence[float]) -> float:
    """Calcula limite superior com 10% de folga sobre o maior valor."""
    if not values:
        return 1.0
    max_value = max(values)
    if max_value <= 0:
        return 1.0
    return max_value * 1.10


def _wrap_condition_labels(labels: Sequence[str]) -> list[str]:
    """Insere quebras de linha para reduzir poluição visual no eixo X."""
    wrapped = []
    replacements = (
        (', sem defesa', ',\nsem defesa'),
...
def _plot_bar_chart(
    ...
    """Gera um gráfico de barras agrupadas em padrão visual de tese.

    O estilo privilegia legibilidade em impressão, consistência tipográfica,
    anotações discretas de valores e contraste robusto para reprodução em
    escala de cinza.
    """
    resolved_path = Path(output_path)
    ensure_directory(resolved_path.parent)
    ...
    labels = _wrap_condition_labels([row['condition_display_name'] for row in rows])
    x_positions = list(range(len(rows)))

    fig, ax = plt.subplots(figsize=(THESIS_FIGURE_WIDTH, THESIS_FIGURE_HEIGHT))
    ...
    if not _has_any_metric(rows, metric_names):
        ...
        ax.set_xticks([])
        fig.savefig(resolved_path)
        fig.savefig(resolved_path.with_suffix('.pdf'))
        plt.close(fig)
        LOGGER.warning(
    ...
    width = 0.74 / max(len(metric_names), 1)
    ...
    for metric_index, metric_name in enumerate(metric_names):
        ...
        for idx, row in enumerate(rows):
            ...
            valid_values.append(metric_value)
        ...
        bars = ax.bar(
            ...
            color=THESIS_COLORS[metric_index % len(THESIS_COLORS)],
            edgecolor='black',
            linewidth=0.6,
            hatch=THESIS_HATCHES[metric_index % len(THESIS_HATCHES)],
            zorder=3,
        )

        decimals = _infer_tick_decimals(metric_names)
        for bar, value in zip(bars, values):
            y_offset = max(_compute_y_axis_upper_bound([value]) * 0.01, 0.002)
            ax.text(
    ...
    y_upper = _compute_y_axis_upper_bound(valid_values)
    ...
    ax.yaxis.set_major_formatter(ticker.StrMethodFormatter('{x:,.2f}' if _infer_tick_decimals(metric_names) == 2 else '{x:,.1f}'))

    legend = ax.legend(loc=legend_loc, ncols=1)
...
def plot_utility_metrics(rows: list[dict[str, Any]], output_dir: str | Path) -> dict[str, Path]:
    ...
    loss_path = _plot_bar_chart(
        ...
        title='Loss de teste por condição experimental',
    ...
    perplexity_path = _plot_bar_chart(
        ...
        title='Perplexidade de teste por condição experimental',
...
def plot_leakage_metrics(rows: list[dict[str, Any]], output_dir: str | Path) -> dict[str, Path]:
    ...
    exact_path = _plot_bar_chart(
        ...
        title='Taxa de exact match do segredo de referência por condição',
    ...
    partial_path = _plot_bar_chart(
        ...
        title='Taxa de partial match do segredo de referência por condição',
    ...
    structured_entity_path = _plot_bar_chart(
        ...
        title='Taxa de geração de entidades estruturadas por condição',
    ...
    canary_path = _plot_bar_chart(
        ...
        title='Taxa de recuperação de canary por condição',
