"""
Script de execução ponta a ponta para a condição com atacante e sem defesa.

Responsabilidades principais:
- Executar sanity check centralizado usando a condição attack_raw.
- Executar avaliação de vazamento sobre o checkpoint do sanity check.
- Executar treino federado com FedAvg usando attack_raw.
...
- É o ponto de entrada prático da condição [B] "com atacante, sem defesa".
- Representa explicitamente o cenário de poisoning com gatilho textual.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description='Executa o pipeline completo da condição com atacante (attack_raw).'
    )
...
def build_base_run_name(explicit_name: str | None) -> str:
    ...
    return f'pipeline_attack_raw_{timestamp}'


def _resolve_clients_per_round() -> int:
    """Resolve dinamicamente quantos clientes podem participar por rodada.

    A função conta os diretórios de clientes disponíveis na condição
    ``attack_raw`` e limita o valor a no máximo 5, preservando o
    comportamento esperado do smoke test quando há menos clientes.

    Args:
        Nenhum.

    Returns:
        Quantidade válida de clientes por rodada, sempre maior ou igual a 1.

    Raises:
        FileNotFoundError: Se o diretório da condição não existir.
        RuntimeError: Se nenhum cliente for encontrado para a condição.
    """
    ...
    if not client_dirs:
        raise RuntimeError(
            'Nenhum cliente encontrado para a condição '
            f"'attack_raw' em {CLIENT_ATTACK_RAW_DATA_DIR}"
        )
...
def main() -> int:
    """Executa o pipeline completo da condição com atacante.
    ...
    Raises:
        FileNotFoundError: Se checkpoints, rodadas ou diretórios esperados não forem encontrados.
    ...
    sanity_attack_output_dir = output_dir / 'attack_sanity_attack_raw'
    command_results.append(
        run_python_module(
    ...
    federated_attack_output_dir = output_dir / 'attack_federated_attack_raw'
    command_results.append(
        run_python_module(
            module='experiments.run_attack_eval',
            args=[
    ...
    summary_path = persist_summary(
        summary,
        summaries_dir / 'attack_raw_pipeline_summary.json',
    )

    LOGGER.info('Pipeline attack_raw concluído. Resumo salvo em %s', summary_path)
    return 0
